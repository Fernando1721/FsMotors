...
alter table produtos add column chassi varchar(255) unique not null after anomodelo;
alter table produtos modify column chassi varchar(255) not null unique;
...
insert into produtos(nomemoto,anomodelo,chassi,descricao,fabricante,quantidade,estoque,estoquemin,localizacao,custo,lucro,venda,idfor)
values ('LANDER250','2021','123456','TRAIL','YAMAHA','1',10,5,'SALA 5',28000,100,56000,1);

describe produtos;
select * from produtos;

ALTER TABLE produtos DROP COLUMN oleo;
...
create table clientes (
    ...
    alter table clientes add column complemento varchar(60) after numero;
...
/**
select idfor as ID,nome as Cliente,fone as Telefone,contato from fornecedores
where fantasia like 'k%';
 */   
...
insert into usuarios(usuario,login,senha,perfil)
values('Renan','rs',md5('123'),'user');

update usuarios set perfil='eofs' where idusu=2;

delete from usuarios where idusu=7;

update usuarios set usuario='Miranda', login='miranda',senha=md5('root'),perfil='admin' where idusu=2;
...
alter table fornecedores add column numero int not null after endereco;
...
insert into fornecedores(cnpj,ie,im,razao,fantasia,site,fone,contato,email,cep,endereco, numero,complemento,bairro,cidade,uf)
values (21672455511,3241388,20447899,'Hyundai Automóveis do Brasil Ltda','Hyundai','Hyundai.com',25637456,'Fs','hyundai@yamaha.com.br','0332678418','Rua Maria Moraes de Morais 48', '78','Loja 4','Centro','São Paulo','SP');

insert into fornecedores(cnpj,ie,im,razao,fantasia,site,fone,contato,email,cep,endereco, numero,complemento,bairro,cidade,uf)
values (21672454158,3241844,2044748,'Kabum do Brasil Ltda','Kabum','kabum.com',25634154,'kaFs','kabum@gmail.com.br','7878212418','Rua Maria oreia de Morais 48', '96','Loja 1','Centro','São Paulo','SP');
